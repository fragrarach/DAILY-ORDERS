DROP VIEW IF EXISTS daily_orders_dev;
CREATE OR REPLACE VIEW daily_orders_dev AS (
    SELECT
    ol.orl_id,
    ol.ord_no,
    oh.ord_date,
...
--     WHERE (oh.ord_date = now()::DATE OR oh.ord_date = now()::DATE - 1)
    WHERE orl_kitmaster_id = 0
        and OH.ORD_NO = 60666
    AND oh.ord_no not in (SELECT * from dblink('dbname=LOG hostaddr=192.168.0.57 port=5493 user=SIGM', 'select * from daily_orders') AS t1(test INTEGER))
